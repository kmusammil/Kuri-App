...
CREATE TABLE public.financial_idempotency_keys (
  ...
  operation_type text NOT NULL CHECK (operation_type IN ('PAYMENT_CREATE','PAYMENT_ALLOCATION')),
  ...
  CHECK (
    ...
    OR (
      operation_type='PAYMENT_ALLOCATION'
...
REVOKE ALL ON public.financial_idempotency_keys FROM PUBLIC, anon, authenticated;

CREATE INDEX financial_idempotency_keys_kuri_idx
  ON public.financial_idempotency_keys(kuri_id, operation_type, created_at DESC);
...
DECLARE
  ...
  normalized_key text := nullif(btrim(idempotency_key),'');
  request_hash text;
