...
begin
  ...
  select d.id into session_id from public.draw_sessions d where d.cycle_id=target_cycle_id for update;
  ...
  select session_id,m.id,
    ...
    (m.status='ACTIVE' and i.status in ('PAID','PAID_LATE')),
    false,null,(select id from public.users where id=auth.uid()),now()
  ...
  set system_eligible=(m.status='ACTIVE' and i.status in ('PAID','PAID_LATE')),
      admin_included=case when e.override then e.admin_included
                          else (m.status='ACTIVE' and i.status in ('PAID','PAID_LATE')) end,
...
create or replace function public.set_draw_pool_entry_for_admin(
  target_entry_id uuid, include_in_draw boolean, reason_text text default null
)
returns void language plpgsql security definer set search_path=public
as $$
declare org_id uuid; session_status public.draw_status; system_eligible_value boolean;
begin
  ...
  set admin_included=include_in_draw,
      ...
      override_reason=case when include_in_draw<>system_eligible_value then nullif(btrim(reason_text),'') else null end,
      modified_by=(select id from public.users where id=auth.uid()),
      modified_at=now()
...
language plpgsql security definer set search_path=public
as $$
declare session_id uuid; candidate_count integer; draw_status_value public.draw_status; cycle_status_value public.cycle_status;
begin
  if auth.uid() is null then raise exception 'You must be signed in.'; end if;
  if selection_count is null or selection_count<1 then raise exception 'Selection count must be at least 1.'; end if;

  select d.id,d.status,c.status into session_id,draw_status_value,cycle_status_value
  from public.draw_sessions d join public.cycles c on c.id=d.cycle_id join public.kuris k on k.id=c.kuri_id
  ...
  select count(*) into candidate_count from public.draw_pool_entries e
  ...
  from (
    select e.membership_id,row_number() over(order by random())::integer rn
    from public.draw_pool_entries e
    where e.draw_session_id=session_id and e.admin_included
    order by random() limit selection_count
  ) picked;
  ...
  from public.draw_selections s join public.memberships m on m.id=s.membership_id join public.people p on p.id=m.person_id
  where s.draw_session_id=session_id order by s.selection_order;
end;
$$;
...
as $$
declare session_id uuid; target_kuri_id uuid; winner_count integer; selected_count integer; cycle_status_value public.cycle_status; draw_status_value public.draw_status;
begin
  if auth.uid() is null then raise exception 'You must be signed in.'; end if;
  if final_membership_ids is null or coalesce(array_length(final_membership_ids,1),0)<1 then
  ...
  select d.id,c.kuri_id,c.status,d.status into session_id,target_kuri_id,cycle_status_value,draw_status_value
  from public.draw_sessions d join public.cycles c on c.id=d.cycle_id join public.kuris k on k.id=c.kuri_id
  where d.cycle_id=target_cycle_id
    and exists (
      select 1 from public.organization_users ou
  ...
  select count(*) into selected_count from public.draw_selections s
  ...
  if exists (
    select 1 from public.draw_selections s join public.memberships m on m.id=s.membership_id
    where s.draw_session_id=session_id and s.membership_id=any(final_membership_ids)
  ...
  select target_cycle_id,m.person_id,'RANDOM_DRAW',(select id from public.users where id=auth.uid()),now()
  ...
  select mw.id,m.id
  from public.monthly_winners mw join public.memberships m on m.person_id=mw.person_id
