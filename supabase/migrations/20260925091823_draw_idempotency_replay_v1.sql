...
BEGIN
  ...
  request_hash:=encode(
    extensions.digest(
      jsonb_build_object(
        'cycle_id',target_cycle_id::text,
        'membership_ids',to_jsonb(final_membership_ids)
      )::text,
      'sha256'
    ),
    'hex'
  ...
  PERFORM 1 FROM public.kuris k WHERE k.id=target_kuri_id FOR UPDATE;

  SELECT d.status INTO draw_status_value
  FROM public.draw_sessions d WHERE d.id=session_id FOR UPDATE;
  ...
  SELECT count(DISTINCT m.person_id) INTO selected_person_count
  ...
  WHERE k.id=target_kuri_id AND c.id=target_cycle_id;
...
REVOKE ALL ON FUNCTION public.run_random_draw_for_admin(uuid,integer,text) FROM PUBLIC,anon;
REVOKE ALL ON FUNCTION public.finalize_draw_for_admin(uuid,uuid[],text) FROM PUBLIC,anon;
GRANT EXECUTE ON FUNCTION public.run_random_draw_for_admin(uuid,integer,text) TO authenticated;
