...
DROP FUNCTION IF EXISTS public.allocate_payment_to_oldest_installments_for_admin(uuid,bigint,text);
...
BEGIN
  ...
  SELECT m.person_id,m.kuri_id
    INTO membership_person_id,membership_kuri_id
  ...
  IF payment_person_id<>membership_person_id THEN RAISE EXCEPTION 'Payment person does not match membership person.'; END IF;
...
REVOKE ALL ON FUNCTION public.allocate_payment_to_oldest_installments_for_admin(uuid,uuid,bigint,text) FROM PUBLIC,anon;
GRANT EXECUTE ON FUNCTION public.allocate_payment_for_admin(uuid,uuid,bigint,text) TO authenticated;
GRANT EXECUTE ON FUNCTION public.allocate_payment_to_oldest_installments_for_admin(uuid,uuid,bigint,text) TO authenticated;
