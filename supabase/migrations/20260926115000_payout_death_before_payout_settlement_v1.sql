-- PAYOUT-004: death-before-payout settlement integration
-- Derive unpaid prize entitlement from the finalized winner, not from payout existence.
-- This prevents omission before payout preparation and double deduction when a pending
-- payout already contains Muppu/expense deductions.
...
RETURNS TABLE(
  ...
  request_cutoff timestamp with time zone,
  death_cutoff date,
...
expenses AS (
  SELECT b.exit_id,
         coalesce(sum(CASE
           WHEN eo.status='UNPAID' AND (
             (b.reason='DEATH' AND b.death_date_verified_at IS NOT NULL AND eo.created_at::date<=b.exit_date)
             OR
             (b.reason<>'DEATH' AND eo.created_at<=coalesce(b.requested_at,b.exit_date::timestamp))
           ) THEN eo.amount ELSE 0 END),0)::bigint amount
  FROM base b
  LEFT JOIN public.expense_obligations eo ON eo.membership_id=b.membership_id
  GROUP BY b.exit_id
),
muppu AS (
  SELECT b.exit_id,
         coalesce(sum(CASE
           WHEN mr.status='UNPAID' AND (
             (b.reason='DEATH' AND b.death_date_verified_at IS NOT NULL AND mr.created_at::date<=b.exit_date)
             OR
             (b.reason<>'DEATH' AND mr.created_at<=coalesce(b.requested_at,b.exit_date::timestamp))
           ) THEN mr.amount ELSE 0 END),0)::bigint amount
  FROM base b
  LEFT JOIN public.muppu_records mr
    ON mr.kuri_id=b.kuri_id
   AND mr.person_id IN (b.original_person_id,b.current_holder_person_id)
  GROUP BY b.exit_id
),
winners AS (
  SELECT b.exit_id,
...
installments AS (
  SELECT b.exit_id,
         coalesce(sum(greatest(ib.amount_due-ib.effective_paid,0)),0)::bigint outstanding_amount
  FROM base b
  JOIN installment_balances ib ON ib.membership_id=b.membership_id
  GROUP BY b.exit_id
)
SELECT b.exit_id,b.membership_id,b.kuri_id,b.original_person_id,b.current_holder_person_id,
       b.membership_number,b.reason,coalesce(b.requested_at,b.exit_date::timestamp),
       ...
       greatest(
         CASE
           WHEN coalesce(w.has_win,false) THEN
             coalesce(w.pending_prize,0)+coalesce(au.unallocated_amount,0)
             -coalesce(ins.outstanding_amount,0)-coalesce(e.amount,0)-coalesce(mu.amount,0)
           ELSE
             coalesce(c.allocated_amount,0)+coalesce(au.unallocated_amount,0)
             -coalesce(e.amount,0)-coalesce(mu.amount,0)
         END,0
       )
FROM base b
...
REVOKE ALL ON FUNCTION public.calculate_membership_exit_financials(uuid) FROM PUBLIC, anon;
