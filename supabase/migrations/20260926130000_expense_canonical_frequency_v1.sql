-- Canonicalize Expense frequency and lock down recurrence RPC execution.
-- Legacy PER_CYCLE remains accepted as an input alias but is no longer stored.
...
BEGIN
  ...
  normalized_frequency :=
    CASE expense_frequency_value::text
...
BEGIN
  ...
  VALUES(
    ...
    (SELECT id FROM public.users WHERE id=actor_id),
    CASE
      WHEN normalized_frequency='ONE_TIME'::public.expense_frequency
      THEN NULL
  ...
  IF recurrence_pattern_value='CUSTOM'::public.expense_recurrence_pattern THEN
    FOREACH schedule_date IN ARRAY custom_schedule_dates LOOP
      INSERT INTO public.expense_rule_schedule_dates(
        expense_rule_id,occurrence_date
      )
...
BEGIN
  ...
  IF rule_row.frequency='ONE_TIME'::public.expense_frequency THEN
    ...
    SELECT
      rule_row.id,rule_row.kuri_id,m.id,NULL,NULL,rule_row.amount
  ...
  IF rule_row.recurrence_pattern='PER_CYCLE'::public.expense_recurrence_pattern THEN
    ...
    SELECT
      rule_row.id,rule_row.kuri_id,m.id,c.id,c.due_date,rule_row.amount
    FROM public.memberships m
    ...
    WHERE m.kuri_id=rule_row.kuri_id
      ...
      AND (
        rule_row.recurrence_start_date IS NULL
        OR c.due_date>=rule_row.recurrence_start_date
      )
      AND (
        rule_row.recurrence_end_date IS NULL
        OR c.due_date<=rule_row.recurrence_end_date
      )
      AND NOT EXISTS (
        SELECT 1
        FROM public.membership_exits me
  ...
  IF rule_row.recurrence_pattern='CUSTOM'::public.expense_recurrence_pattern THEN
    ...
    LOOP
      ...
      SELECT
        rule_row.id,rule_row.kuri_id,m.id,NULL,
  ...
  WHILE cursor_date<=end_date LOOP
    ...
    SELECT
      rule_row.id,rule_row.kuri_id,m.id,NULL,cursor_date,rule_row.amount
    FROM public.memberships m
    WHERE m.kuri_id=rule_row.kuri_id
      AND m.status='ACTIVE'
      AND NOT EXISTS (
...
BEGIN
  ...
  IF rule_row.frequency='ONE_TIME'::public.expense_frequency THEN
    ...
    VALUES(
      rule_row.id,rule_row.kuri_id,target_membership_id,NULL,rule_row.amount,NULL
    )
    ON CONFLICT DO NOTHING;
  ...
  IF rule_row.recurrence_pattern='PER_CYCLE'::public.expense_recurrence_pattern THEN
    ...
    SELECT
      rule_row.id,rule_row.kuri_id,target_membership_id,c.id,
      rule_row.amount,c.due_date
