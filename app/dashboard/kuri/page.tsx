...
export default async function KuriListPage() {
  ...
  if (role !== "MAIN_ADMIN" && role !== "ADMIN") {
    redirect(
      "/dashboard?error=You%20do%20not%20have%20permission%20to%20view%20Kuris.",
    );
  }
        ...
        <div className="flex items-center justify-between gap-4">
          <div>
            ...
            <p className="mt-2 text-sm text-slate-600">
              Create and manage your Kuri schemes.
            </p>
          </div>
              ...
              >
                ...
                <div className="mt-5 grid grid-cols-2 gap-3 text-sm">
                  <div>
                    ...
                    <p className="font-medium">{kuri.start_date}</p>
                  </div>
                  <div>
                    <p className="text-slate-500">Cycles</p>
                    <p className="font-medium">{kuri.number_of_cycles}</p>
                  </div>
                  <div>
                    <p className="text-slate-500">Membership limit</p>
                    <p className="font-medium">{kuri.membership_limit}</p>
                  </div>
                  <div>
                    <p className="text-slate-500">Installment</p>
                    <p className="font-medium">
                      ₹{kuri.installment_amount.toLocaleString("en-IN")}
                    </p>
                  </div>
                </div>
              </Link>
