...
export default async function CycleDetailPage({ params, searchParams }: { params: Promise<{ id: string; cycleId: string }>; searchParams: Promise<{ error?: string }> }) {
  ...
  return <main className="min-h-screen bg-slate-50 px-6 py-10 text-slate-900"><div className="mx-auto max-w-6xl">
    ...
    <section className="mt-6 rounded-2xl border bg-white p-6 shadow-sm"><div className="flex items-center justify-between"><div><h2 className="text-lg font-semibold">Monthly Draw</h2><p className="mt-1 text-sm text-slate-600">Prepare the pool, include or exclude memberships, then run the random draw.</p></div>{!session?<form action={prepareDraw}><input type="hidden" name="cycle_id" value={cycleId}/><input type="hidden" name="kuri_id" value={id}/><button className="rounded-lg bg-slate-900 px-4 py-2.5 text-white">Prepare draw</button></form>:<div className="flex items-center gap-2"><span className="text-sm font-medium">{session.status}</span><form action={prepareDraw}><input type="hidden" name="cycle_id" value={cycleId}/><input type="hidden" name="kuri_id" value={id}/><button className="rounded-lg border px-4 py-2">Reset pool</button></form></div>}</div>
      {poolError?<div className="mt-4 text-sm text-red-700">{poolError.message}</div>:session?<><div className="mt-5 flex items-center gap-4 text-sm"><span>Session: <strong>{session.status}</strong></span><form action={runRandomDraw}><input type="hidden" name="cycle_id" value={cycleId}/><input type="hidden" name="kuri_id" value={id}/><input type="hidden" name="selection_count" value="1"/><button className="rounded-lg bg-slate-900 px-4 py-2 text-white">Run random draw</button></form></div>
      <div className="mt-5 overflow-x-auto"><table className="w-full text-left text-sm"><thead><tr className="border-b text-slate-500"><th className="px-3 py-3">Membership</th><th className="px-3 py-3">Person</th><th className="px-3 py-3">System eligible</th><th className="px-3 py-3">Included</th><th className="px-3 py-3">Override</th><th className="px-3 py-3">Action</th></tr></thead><tbody>{(poolRows??[]).map(e=><tr key={e.entry_id} className="border-b last:border-0"><td className="px-3 py-3">{e.membership_number}</td><td className="px-3 py-3">{e.display_name||e.registered_name}</td><td className="px-3 py-3">{e.system_eligible?"Yes":"No"}</td><td className="px-3 py-3">{e.admin_included?"Yes":"No"}</td><td className="px-3 py-3">{e.override?"Yes":"No"}</td><td className="px-3 py-3"><form action={setPoolEntry}><input type="hidden" name="entry_id" value={e.entry_id}/><input type="hidden" name="cycle_id" value={cycleId}/><input type="hidden" name="kuri_id" value={id}/><input type="hidden" name="include" value={e.admin_included?"false":"true"}/><button className="underline">{e.admin_included?"Exclude":"Include"}</button></form></td></tr>)}</tbody></table></div>
      {selections.length?<div className="mt-5 rounded-xl border p-4"><p className="font-semibold">Random selections</p>{selections.map(s=><p key={s.selection_id} className="mt-2 text-sm">Selection {s.selection_order}: {s.display_name || s.registered_name} · Membership {s.membership_number}</p>)}<form action={finalizeDraw} className="mt-4"><input type="hidden" name="cycle_id" value={cycleId}/><input type="hidden" name="kuri_id" value={id}/><input type="hidden" name="membership_ids" value={selections.map(s=>s.membership_id).join(",")}/><button className="rounded-lg bg-slate-900 px-4 py-2 text-white">Finalize selected winner(s)</button></form></div>:null}</>:null}
    </section>
  </div></main>;
}