...
export default async function CycleDetailPage({ params, searchParams }: { params: Promise<{ id: string; cycleId: string }>; searchParams: Promise<{ error?: string }> }) {
  ...
  return <main className="min-h-screen bg-slate-50 px-6 py-10 text-slate-900"><div className="mx-auto max-w-6xl">
    ...
    <section className="mt-6 rounded-2xl border bg-white p-6 shadow-sm"><div className="flex items-center justify-between"><div><h2 className="text-lg font-semibold">Monthly Draw</h2><p className="mt-1 text-sm text-slate-600">Prepare the pool, include or exclude memberships, then run the random draw.</p></div>{!session?<form action={prepareDraw}><input type="hidden" name="cycle_id" value={cycleId}/><input type="hidden" name="kuri_id" value={id}/><button className="rounded-lg bg-slate-900 px-4 py-2.5 text-white">Prepare draw</button></form>:<div className="flex items-center gap-2"><span className="text-sm font-medium">{session.status}</span><form action={prepareDraw}><input type="hidden" name="cycle_id" value={cycleId}/><button className="rounded-lg border px-4 py-2">Reset pool</button></form></div>}</div>
      {poolError?<div className="mt-4 text-sm text-red-700">{poolError.message}</div>:session?<><div className="mt-5 flex items-center gap-4 text-sm"><span>Session: <strong>{session.status}</strong></span><form action={runRandomDraw}><input type="hidden" name="cycle_id" value={cycleId}/><input type="hidden" name="selection_count" value="1"/><button className="rounded-lg bg-slate-900 px-4 py-2 text-white">Run random draw</button></form></div>
