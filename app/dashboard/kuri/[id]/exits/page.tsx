...
import { createExit, approveExit, settleExit } from "./actions";
...
export default async function MembershipExitsPage({
  ...
  return <main className="min-h-screen bg-slate-50 px-6 py-10 text-slate-900"><div className="mx-auto max-w-6xl">
    ...
    <section className="mt-6 rounded-2xl border bg-white p-6 shadow-sm"><h2 className="text-lg font-semibold">Exit records</h2>{error?<p className="mt-4 text-sm text-red-700">{error.message}</p>:exits?.length?<div className="mt-5 overflow-x-auto"><table className="w-full text-left text-sm"><thead><tr className="border-b text-slate-500"><th className="px-3 py-3">Membership</th><th className="px-3 py-3">Person</th><th className="px-3 py-3">Reason</th><th className="px-3 py-3">Contributed</th><th className="px-3 py-3">Refund</th><th className="px-3 py-3">Policy</th><th className="px-3 py-3">Status</th><th className="px-3 py-3">Action</th></tr></thead><tbody>{exits.map(e=><tr key={e.exit_id} className="border-b last:border-0"><td className="px-3 py-3">{e.membership_number}</td><td className="px-3 py-3">{e.display_name||e.registered_name}</td><td className="px-3 py-3">{e.reason}</td><td className="px-3 py-3">₹{Number(e.amount_contributed).toLocaleString("en-IN")}</td><td className="px-3 py-3">₹{Number(e.refund_amount).toLocaleString("en-IN")}</td><td className="px-3 py-3">{e.refund_policy}</td><td className="px-3 py-3">{e.status}</td><td className="px-3 py-3 flex gap-3">{e.status==="PENDING"?<form action={approveExit}><input type="hidden" name="kuri_id" value={id}/><input type="hidden" name="exit_id" value={e.exit_id}/><button className="underline">Approve</button></form>:null}{e.status==="APPROVED"?<form action={settleExit}><input type="hidden" name="kuri_id" value={id}/><input type="hidden" name="exit_id" value={e.exit_id}/><button className="underline">Settle</button></form>:null}</td></tr>)}</tbody></table></div>:<p className="mt-4 text-sm text-slate-600">No exit records.</p>}</section>
  </div></main>;
}